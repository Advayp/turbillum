...
pub(crate) fn extract_digits(s: &str) -> Result<(&str, &str), String> {
    take_while1(|c| c.is_ascii_digit(), s, "expected number".to_string())
}
...
mod tests {
    ...
    #[test]
    fn do_not_extract_digits_when_possible() {
        assert_eq!(extract_digits("abcd"), Err("expected number".to_string()))
    }
    ...
    #[test]
    fn extract_indent_from_a_string() {
        assert_eq!(extract_ident("asdfasdf stop"), Ok((" stop", "asdfasdf")));
    }

    #[test]
    fn extract_alphanumeric_indent() {
        assert_eq!(extract_ident("hello123()"), Ok(("()", "hello123")));
    }

    #[test]
    fn do_not_extract_identifier_if_starting_with_number() {
        assert_eq!(
            extract_ident("123asbdf"),
            Err("expected identifier".to_string())
        );
    }

    #[test]
    fn extract_tag_from_statement() {
        assert_eq!(tag("def", "def a"), Ok(" a"));
    }
